...
// --- Signature verification (Polar uses Standard Webhooks spec: Svix-compatible HMAC-SHA256) ---
async function verifyPolarSignature(
  ...
  // Polar's secret format: "polar_whs_<base64url-encoded-random-bytes>"
  // Strip the "polar_whs_" prefix, then base64-decode the remainder.
  // (Standard Webhooks uses base64, but Polar may use base64url — we handle both.)
  let base64 = secret.replace('polar_whs_', '').replace('whsec_', '');
  // Convert base64url to base64 (replace URL-safe chars, pad if needed)
  base64 = base64.replace(/-/g, '+').replace(/_/g, '/');
  while (base64.length % 4 !== 0) {
    base64 += '=';
  }
  const secretBytes = Uint8Array.from(atob(base64), (c) => c.charCodeAt(0));
